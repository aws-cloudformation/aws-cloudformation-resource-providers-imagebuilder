...
public class ReadHandler extends BaseHandler<CallbackContext> {
    ...
    private ProgressEvent<ResourceModel, CallbackContext> fetchImageRecipeAndAssertExists() {
        ...
        GetImageRecipeResponse response = null;
        try {
            response = proxy.injectCredentialsAndInvokeV2(RequestUtil.generateReadImageRecipeRequest(model),
                    ClientBuilder.getImageBuilderClient()::getImageRecipe);
